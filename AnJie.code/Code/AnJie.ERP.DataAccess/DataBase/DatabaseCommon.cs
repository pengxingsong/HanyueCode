...
namespace AnJie.ERP.DataAccess
{
    public class DatabaseCommon
    {
        ...
        public static DbParameter[] GetParameter<T>(T entity)
        {
            ...
            foreach (PropertyInfo pi in props)
            {
                if (pi.GetValue(entity, null) != null)
                {
                    ...
                    parameter.Add(DbFactory.CreateDbParameter(DbHelper.DbParmChar + pi.Name, pi.GetValue(entity, null),
                        dbtype));
                }
            }
        ...
        public static object GetKeyField<T>()
        {
            ...
            {
                var attrKeyField = attr as PrimaryKeyAttribute;
                if (attrKeyField != null)
                {
                    keyField = attrKeyField.Name;
                }
            }
            return keyField;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static object GetTableName<T>()
        {
            Type objTye = typeof(T);
            string tableName = objTye.Name;
            foreach (Attribute attr in objTye.GetCustomAttributes(true))
            {
                var keyField = attr as TableNameAttribute;
                if (keyField != null)
                {
                    tableName = keyField.Name;
                }
            }
            return tableName;
        }
        ...
        {
            Assembly asmb =
                Assembly.LoadFrom(System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin",
                    "AnJie.ERP.Entity.dll"));
        ...
        public static object GetKeyFieldValue<T>(T entity)
        {
            ...
            PropertyInfo property = objTye.GetProperty(keyField);
        ...
        {
            return
                new StringBuilder("Delete From " + tableName + " Where " + pkName + " = " + DbHelper.DbParmChar + pkName +
                                  "");
        }
