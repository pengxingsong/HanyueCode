...
namespace AnJie.ERP.DataAccess
{
    ...
    public class DatabaseReader
    {
        ...
        /// <summary>
        ///  将IDataReader转换为DataTable,表头字段大小写不变化
        /// </summary>
        /// <param name="dr"></param>
        /// <returns></returns>
        public static DataTable ReaderToDataTableOriginal(IDataReader dr)
        {
            using (dr)
            {
                DataTable objDataTable = new DataTable("Table");
        ...
        public static List<T> DataTableToList<T>(DataTable dt)
        {
            using (dt)
            {
                List<string> field = new List<string>(dt.Columns.Count);
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    field.Add(dt.Columns[i].ColumnName.ToLower());
                }
                List<T> list = new List<T>();
                foreach (DataRow item in dt.Rows)
                {
                                ...
                                .GetProperties(BindingFlags.GetProperty | BindingFlags.Public | BindingFlags.Instance))
                    {
                        if (field.Contains(property.Name.ToLower()))
                        {
                            if (!IsNullOrDBNull(item[property.Name]))
                            {
                                property.SetValue(model, HackType(item[property.Name], property.PropertyType), null);
                            }
                        }
                    }
                    list.Add(model);
        ...
        public static T ReaderToModel<T>(IDataReader dr)
        {
            using (dr)
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        if (field.Contains(pi.Name.ToLower()))
                        {
        ...
        public static bool IsNullOrDBNull(object obj)
        {
            ...
            return obj is DBNull ? true : false;
        }
    }
}