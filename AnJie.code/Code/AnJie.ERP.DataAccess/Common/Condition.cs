...
namespace AnJie.ERP.DataAccess.Common
{
    /// <summary>
    /// 多条件动态查询属性
    /// </summary>
    ...
    public class ConditionBuilder
    {
        ...
        /// <returns></returns>
        public static string GetWhereSql(IList conditions, out List<DbParameter> parameter, string orderField = "",
            string orderType = "")
        {
            DateTime startTime;
            DateTime endTime;
            ...
            foreach (Condition item in conditions)
            {
                if (item.ParamValue == null)
                    continue;
                string Logic = "";
                if (string.IsNullOrEmpty(item.Logic)) Logic = "";
                else Logic = item.Logic == "AND" ? "AND" : "OR";
                ...
                switch (item.Operation)
                {
                    ...
                    case ConditionOperate.NotEqual:
                        sbWhere.Append(" " + item.LeftBrace + fieldName + " <> " + ParamKey + paramName +
                                       item.RightBrace + " " + Logic);
                        ParamList.Add(DbFactory.CreateDbParameter(ParamKey + paramName, item.ParamValue));
                        break;
                    ...
                    case ConditionOperate.NotNull:
                        sbWhere.Append(string.Format(" " + item.LeftBrace + "{0} is not null ", fieldName) +
                                       item.RightBrace + " " + Logic);
                        break;
                    case ConditionOperate.Like:
                        sbWhere.Append(" " + item.LeftBrace + fieldName + " like " + ParamKey + paramName +
                                       item.RightBrace + " " + Logic);
                        ParamList.Add(DbFactory.CreateDbParameter(ParamKey + paramName, "%" + item.ParamValue + "%"));
                        break;
                    case ConditionOperate.NotLike:
                        sbWhere.Append(" " + item.LeftBrace + fieldName + " not like " + ParamKey + paramName +
                                       item.RightBrace + " " + Logic);
                        ParamList.Add(DbFactory.CreateDbParameter(ParamKey + paramName, "%" + item.ParamValue + "%"));
                        break;
                    ...
                    case ConditionOperate.Yesterday:
                        ...
                        sbWhere.Append(
                            string.Format(
                                " " + item.LeftBrace + "{0} between " + ParamKey + "start{1}  AND " + ParamKey +
                                "end_{2}" + item.RightBrace + "", fieldName, paramName, paramName) + " " + Logic);
                        ParamList.Add(DbFactory.CreateDbParameter(ParamKey + string.Format("start{0}", paramName),
                            startTime));
                        ParamList.Add(DbFactory.CreateDbParameter(ParamKey + string.Format("end_{0}", paramName),
                            endTime));
                        break;
                    ...
                    case ConditionOperate.ThisWeek:
                        startTime =
                            CommonHelper.GetDateTime(
                                DateTime.Now.AddDays(1 - Convert.ToInt32(DateTime.Now.DayOfWeek)).ToString("yyyy-MM-dd") +
                                " 00:00");
                        endTime = CommonHelper.GetDateTime(startTime.AddDays(6).ToString("yyyy-MM-dd") + " 23:59");
                        sbWhere.Append(
                            string.Format(
                    ...
                    case ConditionOperate.LastMonth:
                        startTime =
                            CommonHelper.GetDateTime(DateTime.Now.AddMonths(-1).ToString("yyyy-MM-01") + " 00:00");
                        endTime =
                            CommonHelper.GetDateTime(
                    ...
                    case ConditionOperate.ThisMonth:
                        ...
                        endTime =
                            CommonHelper.GetDateTime(DateTime.Now.AddMonths(1).AddDays(-1).ToString("yyyy-MM-dd") +
                                                     " 23:59");
                        sbWhere.Append(
                            string.Format(
                                " " + item.LeftBrace + "{0} between " + ParamKey + "start{1}  AND " + ParamKey +
                    ...
                    case ConditionOperate.NextMonth:
                        ...
                        endTime =
                            CommonHelper.GetDateTime(
                                DateTime.Parse(DateTime.Now.ToString("yyyy-MM-01"))
                                    .AddMonths(2)
                                    .AddDays(-1)
                                    .ToString("yyyy-MM-dd") + " 23:59");
                        sbWhere.Append(
                            string.Format(
                                " " + item.LeftBrace + "{0} between " + ParamKey + "start{1}  AND " + ParamKey +
                                "end_{2}" + item.RightBrace + "", fieldName, paramName, paramName) + " " + Logic);
                    ...
                    case ConditionOperate.BeforeDay:
                        startTime =
                            CommonHelper.GetDateTime(
                                DateTime.Now.AddDays(double.Parse("-" + item.ParamValue.ToString())) + " 00:00");
                        endTime = CommonHelper.GetDateTime(DateTimeHelper.GetToday() + " 23:59");
                        sbWhere.Append(
                            string.Format(
                                " " + item.LeftBrace + "{0} between " + ParamKey + "start{1}  AND " + ParamKey +
                    ...
                    case ConditionOperate.AfterDay:
                        startTime =
                            CommonHelper.GetDateTime(DateTime.Now.AddDays(double.Parse(item.ParamValue.ToString())) +
                                                     " 00:00");
                        endTime = CommonHelper.GetDateTime(DateTimeHelper.GetToday() + " 23:59");
                        sbWhere.Append(
                            string.Format(
