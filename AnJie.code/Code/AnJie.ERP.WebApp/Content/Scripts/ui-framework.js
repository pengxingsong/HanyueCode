...
/********
URL地址栏参数拼接，返回字符串（值经过escape处理）
格式[["名称","值"],...]
**********/
function SetUrlParameter(paraArry) {
    var str = [];
    if (paraArry) {
        for (var i = 0; i < paraArry.length; i++) {
            str.push(paraArry[i][0] + "=" + escape(paraArry[i][1]));
        }
    }
    if (str)
        str = "?" + str.join("&");
...
function GetParameterJson(tableId) {
    ...
    $("#" + tableId + " tr").find("td").find("input,select").each(function () {
        ...
        var pkValue = $("#" + pkId).val();
        parameter += '"' + pkId + '"' + ":" + '"' + $.trim(pkValue) + '",';
    });
        ...
        $(tableId + " tbody tr").each(function () {
            ...
            $(this)
                ...
                .each(function() {
                    ...
                    var rowValue = "";
                    if ($("#" + rowId).attr("type") === "checkbox") {
                        if ($("#" + rowId).attr("checked")) {
                            rowValue = "1";
                        } else {
                            rowValue = "0";
                        }
                    ...
                    var array = new Array();
                    array = rowId.split("➩"); //字符分割
                    tdjson += '"' + array[0] + '"' + ":" + '"' + $.trim(rowValue) + '",';
                });
...
function GetTableJson(tableId, rowKey) {
    var trjson = "";
    if ($(tableId + " tbody tr").length > 0) {
        $(tableId + " tbody tr").each(function () {
            var tdjson = "";
            ...
            $(this)
                ...
                .each(function () {
                    ...
                    var rowValue = "";
                    if ($("#" + rowId).attr("type") === "checkbox" ) {
                        if($("#" + rowId).attr("cbvaltype") === "int"){
                            rowValue = $("#" + rowId).attr("checked") ? "1" : "0";
                        }
                        if ($("#" + rowId).attr("cbvaltype") === "bool") {
                            rowValue = $("#" + rowId).attr("checked") ? "true" : "false";
                        }
                    } else {
                        rowValue = $("#" + rowId).val();
                    }
                    ...
                    array = rowId.split("➩"); //字符分割
                    tdjson += '"' + array[0] + '"' + ":" + '"' + $.trim(rowValue) + '",';
                    if (array[0] === rowKey && $.trim(rowValue) !== "") {
                        validRow = true;
                    }
                });
            if (validRow) {
...
弹出对话框（带：确认按钮、取消按钮）
*/
function openDialogWithCancel(url, _id, _title, _width, _height, callBack, cancelCallBack) {
    Loading(true);
    top.$.dialog({
        id: _id,
        width: _width,
...
function operationConfig(operation, url, parm, count) {
    if (count == undefined) {
        count = 1;
    }
    confirmDialog("系统提示", "注：您确定要" + operation + " " + count + " 条记录？", function (r) {
        if (r) {
...
function BindDropItem(ControlId, Code, Memo) {
    ...
    getAjax('/SystemModule/DataDictionary/BinDingItemsJson', { Code: Code }, function (data) {
...
function PartialButton() {
    var jsonData = "";
    var toolbarHtml = "";
    var toolbarChildrenCount = 0;
    AjaxJson("/Utility/LoadButton", {}, function (data) {
        jsonData = data;
        $.each(data, function (i) {
            if (data[i].ParentId === "0" && data[i].Category === "1") {
                var toolbarChildrenList = toolbarChildren(data[i].ButtonId, data)
                if (toolbarChildrenCount > 0) {
                    toolbarHtml += "<a id=\"" + data[i].Code + "\" class=\"tools_btn dropdown\">";
                    toolbarHtml += "<div style=\"float: left;\"><div class=\"icon\"><img src=\"../../Content/Images/Icon16/" + data[i].Icon + "\" /></div><div class=\"text\">" + data[i].FullName + "</div></div>";
                    toolbarHtml += "<div class=\"dropdown-icon\"><img src=\"../../Content/Images/dropdown-icon.png\" /></div>";
                    toolbarHtml += "<div class=\"dropdown-data\"><i></i><span></span>";
                    toolbarHtml += "    <ul>";
                    toolbarHtml += toolbarChildrenList;
                    toolbarHtml += "    </ul>";
                    toolbarHtml += "</div>";
                    toolbarHtml += "</a>";
                } else {
                    ...
                    toolbarHtml += "<a id=\"" + data[i].Code + "\" " + title + " onclick=\"" + data[i].JsEvent + "\" class=\"tools_btn\"><span><b style=\"background: url('../../Content/Images/Icon16/" + data[i].Icon + "') 50% 4px no-repeat;\">" + data[i].FullName + "</b></span></a>";
                }
                if (data[i].Split === 1) {
                    toolbarHtml += "<div class=\"tools_separator\"></div>";
                }
            }
        });
    });
    $(".tools_bar .PartialButton").html(toolbarHtml);
    function toolbarChildren(buttonId, data) {
        var children = "";
        toolbarChildrenCount = 0;
        $.each(data, function (i) {
            if (data[i].ParentId === buttonId) {
                var title = "title=\"" + data[i].Remark + "\"";
                if (data[i].Remark === "") { title = ""; }
                children += "<li id=\"" + data[i].Code + "\" " + title + " onclick=\"" + data[i].JsEvent + "\"><img src=\"../../Content/Images/Icon16/" + data[i].Icon + "\" />" + data[i].FullName + "</li>";
                toolbarChildrenCount++;
            }
        });
        return children;
    }
