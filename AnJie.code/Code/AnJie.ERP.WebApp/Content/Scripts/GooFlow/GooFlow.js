...
function GooFlow(bgDiv, property) {
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            e.data.inthis.addNode(e.data.inthis.$id + "_node_" + e.data.inthis.$max, { name: "新建节点" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, css: '', img: '', });
            e.data.inthis.$max++;
        });
        //划线时用的绑定
...
GooFlow.prototype = {
    ...
    //只把本次编辑流程图中作了修改(包括增删改)的元素导出到一个变量中,以方便用户每次编辑载入的流程图后只获取修改过的数据
    ...
    //修改元素的ID,一般用于快速保存后,将后台返回新元素的ID更新到页面中;type为元素类型(节点,连线,区块)
