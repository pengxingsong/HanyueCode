...
namespace AnJie.ERP.Business
{
    ...
    {
        private readonly BaseDataBaseBLL _baseDatabasebll = new BaseDataBaseBLL();
        ...
        public string FindModelsType(string name)
        {
            ...
            else if (name == "char" || name == "varchar" || name == "nvarchar2" || name == "text" || name == "nchar" ||
                     name == "nvarchar" || name == "ntext")
            {
                return "string";
            }
            else if (name == "bit")
        ...
        public string CodeBuilder_Entity(string table, string Key, DataTable Entity_dt)
        {
            ...
            //sb_Entity.Append("//=====================================================================================\r\n\r\n");
            sb_Entity.Append("using AnJie.ERP.DataAccess.Attributes;\r\n");
            sb_Entity.Append("using AnJie.ERP.Utilities;\r\n");
        ...
        public string GetCodeBuilderBusiness(string table)
        {
            ...
            //sb_Business.Append("//=====================================================================================\r\n\r\n");
            sb_Business.Append("using AnJie.ERP.Entity;\r\n");
            sb_Business.Append("using AnJie.ERP.Repository;\r\n");
            sb_Business.Append("using AnJie.ERP.Utilities;\r\n");
        ...
        public string GetCodeBuilderFrom(string table, string FromJson, int ColumnCount, string FormCss)
        {
            ...
            sb_From.Append("            AjaxJson(\"/" + AreasName + "/" + StringHelper.DelLastLength(ControllerName, 10) +
                           "/SetForm\", { KeyValue: KeyValue }, function (data) {\r\n");
            ...
            sb_From.Append("        AjaxJson(\"/" + AreasName + "/" + StringHelper.DelLastLength(ControllerName, 10) +
                           "/SubmitForm?KeyValue=\" + KeyValue, postData, function (data) {\r\n");
            ...
            sb_From.Append("    " +
                           Base_FormAttributeBll.Instance.CreateBuildFormTable(ColumnCount, ListData)
                               .Replace("Build_", ""));
            sb_From.Append("</form>\r\n");
            WriteCodeBuilder(
                table + "\\" + StringHelper.DelLastLength(ControllerName, 10) + "\\" + PageFormName + ".cshtml",
                sb_From.ToString());
        ...
        public string GetCodeBuilderFromDetail(string table, string fromJson, int columnCount, string formCss)
        {
            ...
            sbFrom.Append("@{\r\n");
            sbFrom.Append("    ViewBag.Title = \"" + ClassName + "》明细页面\";\r\n");
            sbFrom.Append("    Layout = \"~/Views/Shared/_LayoutForm.cshtml\";\r\n");
            sbFrom.Append("}\r\n");
            sbFrom.Append("<script type=\"text/javascript\">\r\n");
            sbFrom.Append("    $(function () {\r\n");
            sbFrom.Append("        InitControl();\r\n");
            sbFrom.Append("    })\r\n");
            sbFrom.Append("    //得到一个对象实体\r\n");
            sbFrom.Append("    function InitControl() {\r\n");
            sbFrom.Append("        if (!!GetQuery('KeyValue')) {\r\n");
            sbFrom.Append("            AjaxJson(\"/" + AreasName + "/" + StringHelper.DelLastLength(ControllerName, 10) +
                           "/SetForm\", { KeyValue: GetQuery('KeyValue') }, function (data) {\r\n");
            sbFrom.Append("                SetWebControls(data);\r\n");
            sbFrom.Append("            });\r\n");
            sbFrom.Append("        }\r\n");
            sbFrom.Append("    }\r\n");
            sbFrom.Append("</script>\r\n");
            sbFrom.Append("<form id=\"form1\" style=\"margin: 1px\">\r\n");
            var listData = (from itementity in fromJson.JonsToList<Base_FormAttribute>()
                where itementity.Enabled == 1
                orderby itementity.SortCode ascending
                select itementity).ToList<Base_FormAttribute>();
            sbFrom.Append("    " +
            ...
            sbFrom.Append("</form>\r\n");
            WriteCodeBuilder(
                table + "\\" + StringHelper.DelLastLength(ControllerName, 10) + "\\" + PageFormDetailName + ".cshtml",
                sbFrom.ToString());
        ...
        public string GetCodeBuilderList(string table,
            string showFieldJson, string allowOrder, string orderType, string orderField, string allowPageing,
            string pageSize, string pageLayout, Hashtable method)
        {
            string PrimaryKeyColumns = _baseDatabasebll.GetPrimaryKey(table);
            ...
            var query = from entity in ListData
                orderby entity.SortCode ascending
                /*descending*/
                select entity;
            if (ListData != null)
            {
                foreach (JqGridColumn item in query)
                {
                    ...
                    if (Enabled == 1)
                    {
                        sb_colModel.Append("                { label: '" + propertyName + "', name: '" + controlId +
                                           "', index: '" + controlId + "', width: " + width + ", align: '" + align +
                                           "',sortable: " + sortable + " " + IsShow_Field(hidden) + " },\r\n");
                    }
                }
            }
            sb_List.Append("@{\r\n");
            ...
            if (allowPageing == "1")
            {
                sb_List.Append("        url: \"@Url.Content(\"~/" + AreasName + "/" +
                               StringHelper.DelLastLength(ControllerName, 10) + "/GridPageJson\")\",\r\n");
            }
            else
            {
                sb_List.Append("        url: \"@Url.Content(\"~/" + AreasName + "/" +
                               StringHelper.DelLastLength(ControllerName, 10) + "/GridJson\")\",\r\n");
            }
            ...
            if (method["AllowInsert"].ToString() == "checked")
            {
                ...
                sb_List.Append("        var url = \"/" + AreasName + "/" +
                               StringHelper.DelLastLength(ControllerName, 10) + "/Form\"\r\n");
                sb_List.Append("        openDialog(url, \"Form\", \"新增" + ClassName +
                               "\", 770, 395, function (iframe) {\r\n");
            ...
            if (method["AllowUpdate"].ToString() == "checked")
            {
                ...
                sb_List.Append("            var url = \"/" + AreasName + "/" +
                               StringHelper.DelLastLength(ControllerName, 10) + "/Form?KeyValue=\" + KeyValue;\r\n");
                sb_List.Append("            openDialog(url, \"Form\", \"编辑" + ClassName +
                               "\", 770, 395, function (iframe) {\r\n");
                sb_List.Append("                top.frames[iframe].AcceptClick();\r\n");
                sb_List.Append("            });\r\n");
                sb_List.Append("        }\r\n");
            ...
            if (method["AllowDelete"].ToString() == "checked")
            {
                ...
                sb_List.Append("        var KeyValue = GetJqGridRowValue(\"#gridTable\", \"" + PrimaryKeyColumns +
                               "\");\r\n");
                sb_List.Append("        if (IsSelectData(KeyValue)) {\r\n");
                sb_List.Append("            var delparm = 'KeyValue=' + KeyValue;\r\n");
                sb_List.Append("            delConfig('/" + AreasName + "/" +
                               StringHelper.DelLastLength(ControllerName, 10) +
                               "/Delete', delparm, KeyValue.split(\",\").length);\r\n");
                sb_List.Append("        }\r\n");
                sb_List.Append("    }\r\n");
            }
            if (method["AlloLookup"].ToString() == "checked")
            {
                ...
                sb_List.Append("        if (IsChecked(KeyValue)) {\r\n");
                sb_List.Append("            var url = \"/" + AreasName + "/" +
                               StringHelper.DelLastLength(ControllerName, 10) + "/Detail?KeyValue=\" + KeyValue;\r\n");
                sb_List.Append("            Dialog(url, \"Detail\", \"" + ClassName +
                               "明细\", 820, 500, function (iframe) {\r\n");
                sb_List.Append("                top.frames[iframe].AcceptClick();\r\n");
                sb_List.Append("            });\r\n");
                sb_List.Append("        }\r\n");
                sb_List.Append("    }\r\n");
            ...
            WriteCodeBuilder(
                table + "\\" + StringHelper.DelLastLength(ControllerName, 10) + "\\" + PageListName + ".cshtml",
                sb_List.ToString());
